...
export default function BottomNav() {
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-800 border-t border-slate-200 dark:border-gray-700 z-50 safe-area-pb">
        ...
        {items.map(item => {
          ...
          return (
            ...
            >
              ...
              <span className={`text-[10px] font-bold ${isActive ? 'text-blue-600 dark:text-blue-400' : ''}`}>{item.label}</span>
            </Link>
          );
        })}
      </div>
